"""
Landing (flap phase 2) sensitivity post-processing — gapped-flap geometry
counterpart to `post/v2_continuous/plot_landing_sensitivities.py`.

Auto-discovers sweep case IDs from project `prj-0cd29981` by name pattern
`gap40_LD_{sweep}_{tok}` and the parent case (`*_parent`).
...
PROJECT_ID  = "prj-0cd29981-d281-442a-984f-06562abc1f39"   # tsangpo_v2_landing_gapped40
...
def main():
    ...
    # Inherit continuous-flap masks; adjust if gap40 stall onset differs.
    mask_a = a <= 8.0
    mask_h = (h >= 10.0) & (h <= 40.0)
    mask_t = t <= 25.0
    ...
    print('\n=== Landing gap40 sensitivities (about α=+8°, θ_ht=-6°, T_mult=+12) ===')
    ...
    fig.suptitle("Tsangpo v2 GAPPED40 LANDING calibration sweeps "
                 f"— BO α=+8°, θ_ht=-6°, T_mult=+12 — V={P.V_LANDING_M_S} m/s, γ={P.DESCENT_ANGLE_DEG:+.0f}°",
                 fontsize=11, y=0.995)
    fig.savefig(OUT / "landing_sensitivities.png", dpi=160); plt.close(fig)
    ...
    ax.set_title(f"gap40 landing thrust vs drag — γ={P.DESCENT_ANGLE_DEG:.0f}°, V={P.V_LANDING_M_S:.1f} m/s")
    ax.grid(True, alpha=0.3); ax.legend()
    fig2.tight_layout(); fig2.savefig(OUT / "landing_thrust_balance.png", dpi=160); plt.close(fig2)
    ...
    try:
        ...
        sm = -dCMy[0]/dCL[0]
        print(f"Refined gap40 landing trim: α={ALPHA_B+dx[0]:+.2f}°, θ_ht={THETA_HT_B+dx[1]:+.2f}°, T_mult={T_B+dx[2]:+.2f}")
